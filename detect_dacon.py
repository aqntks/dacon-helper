        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    # if (int(cls + 1) == 1 or int(cls + 1) == 2) and conf >= 0.4:
                    results['file_name'].append(path.split('\\')[-1].split('.')[0] + '.json')
                    class_id = int(cls + 1)
                    confidence = float(conf)
                    point1_x = float(xyxy[0])
                    point1_y = float(xyxy[1])
                    point2_x = float(xyxy[2])
                    point2_y = float(xyxy[1])
                    point3_x = float(xyxy[2])
                    point3_y = float(xyxy[3])
                    point4_x = float(xyxy[0])
                    point4_y = float(xyxy[3])
                    ...
                    # elif (int(cls + 1) == 3 or int(cls + 1) == 4) and conf >= 0.3:
                    #     results['file_name'].append(path.split('\\')[-1].split('.')[0] + '.json')
                    #     class_id = int(cls + 1)
                    #     confidence = float(conf)
                    #     point1_x = float(xyxy[0])
